...
data_PG_2023 <- subset(Data_PG_2023, select = -c(player_name))

ggpairs(data, title = "Scatterplot Matrix of the Features")



n_PG <- neuralnet(drafted ~ conf + GP + Min_per + ORtg + usg + eFG + TS_per + ORB_per + DRB_per +
                    AST_per + TO_per + FTM + FTA + FT_per + twoPM + twoPA + twoP_per + TPM + TPA + 
                    TP_per + blk_per + stl_per + ftr + yr + porpag + adjoe + pfr + pid + Rec.Rank +
                    X.ast.tov + X.rimmade + X.rimmade.rimmiss + X.midmade + X.midmade.midmiss + 
                    X.rimmade..rimmade.rimmiss. + X.midmade..midmade.midmiss. + X.dunksmade + X.dunksmiss.dunksmade +
                    X.dunksmade..dunksmade.dunksmiss. + X.drtg + adrtg + X.dporpag + X.stops + X.bpm + X.obpm +
                    X.dbpm + X.gbpm + mp + ogbpm + oreb + dreb + treb + ast + stl + blk + pts + ht_cm,
               data = Data_PG,
               hidden = c(12,7),
               rep=1)

plot(n_PG,col.hidden = 'darkgreen',     
...
training <- data[ind==1,1:58]
test <- data[ind==2, 1:58]
trainingtarget <- data[ind==1, 59]
testtarget <- data[ind==2, 59]
str(trainingtarget)
...
## Model creation : Naive !

model <- keras_model_sequential()
model %>%
  layer_dense(units = 5, activation = 'relu', input_shape = c(58)) %>%
  layer_dense(units = 1)
...
model %>%
  layer_dense(units = 100, activation = 'relu', input_shape = c(58)) %>%
