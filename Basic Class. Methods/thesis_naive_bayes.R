...
#REPLACE NULL AND NA VALUES BY 0 -- CAN BE MODIFIED LATER : 
...
model <- naive_bayes(as.factor(drafted) ~ ., data = train)
summary(model)
...
data.frame( R2 = R2(as.numeric(predict_reg), test$drafted),
            RMSE = RMSE(as.numeric(predict_reg), test$drafted),
            MAE = MAE(as.numeric(predict_reg), test$drafted))
